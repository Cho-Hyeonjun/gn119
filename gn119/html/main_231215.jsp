...
//---------------------------------------------------------------- 경상남도지원사업 달력
    ...
    int gnJiwonCalCnt = 0;	// 데이터 개수
    
    Calendar cal = Calendar.getInstance();
    SimpleDateFormat smdf = new SimpleDateFormat("yyyy-MM-dd");
    SimpleDateFormat smdf2 = new SimpleDateFormat("yyyyMMdd");
...
//오늘 날 +8일
    ...
    dayPlusEight = smdf.format(calPlusDayEight.getTime());
...
//오늘 날 +9일
    ...
    dayPlusNine = smdf.format(calPlusDayNine.getTime());
...
//---- 오늘날짜데이터 가져오기
    try{
        ...
        whereSql += "AND TO_DATE(?, 'yyyyMM') BETWEEN TO_DATE(REG_START_MONTH, 'yyyyMM') AND TO_DATE(REG_END_MONTH, 'yyyyMM')	";
        calDayStr = year + "" + (Integer.parseInt(month)<10 ? "0" + Integer.parseInt(month) : month);
        setList.add(calDayStr);
    }catch(NullPointerException | NumberFormatException e){
        //e.printStackTrace();
        setList.add(compareMonth.format(compareCurrentMonth));

    }

    setObj = new String[setList.size()];
    ...
    // 날짜에 해당하는 데이터 개수
     try{
    	gnJiwonCalCnt = jdbcTemplate.queryForObject(gnJiwonCalCount2(whereSql).toString(), Integer.class, setObj);
    }catch(NullPointerException e){
		System.out.println("지원사업연간일정 nullError");
    }
    
    // 날짜에 해당하는 데이터
    try{
        gnJiwonCalList = jdbcTemplate.query(gnJiwonCal2(whereSql), new Db(), setObj);
    }catch(NullPointerException e){
		System.out.println("지원사업연간일정 nullError");
    }
...
//---------------------------------------------------------------- 기업홍보
    List<Map<String,Object>> giupList = null;
    giupList = jdbcTemplate.query(giupHongBoListForMain(), new Db(), 8, 1);
    String giupViewPage = "/giup/board/view.gyeong?boardId=BBS_0000913&menuCd=DOM_000004604006001000";
                                ...
                                <li class="swiper-slide">
                                    <a href="/giup/index.gyeong?menuCd=DOM_000004603001001000&menuType=U&busiSupportCd=<%=getString(ob,"BUSI_SUPPORT_CD")%>">
                                        ...
                                        <h3 class="board_tit">
                                            ...
                                            <br/>
                                            <span class="ellipsis tit"><%=getString(ob,"BUSI_SUPPORT_NM")%></span>
                                        </h3>
                                        <p class="board_txt">
                                            <strong>접수기간</strong>
                                        ...
                                        <p class="board_txt"><strong>수행기관</strong>
                                            <span class="board_cont">
															<%=getString(ob, "EXCLNSTT_NM")%>
														</span>
                                        </p>
                                        <%-- <span class="board_date">조회수 : <%=getString(ob, "VIEW_COUNT") %></span> --%>
                                        <p class="board_txt"><strong>문의처</strong>
                                            <span class="board_cont">
															<%=getString(ob, "INQUIRY_INFO")%>
														</span>
                                        </p>
                                        <%-- <span class="board_date">조회수 : <%=getString(ob, "VIEW_COUNT") %></span> --%>
                                    </a>
                ...
                <div class="calendar_area">
                    <div class="calendar_cont">
                                ...
                                <tr>
                                    ...
                                    <td class="calTd" id="<%=calDayStr%>">
                                        <% if("0".equals(calDayStr.substring(8,9))){ /* 일자가 10보다 작을때 (일이 한자리 수 일때) */%>
											<a href="javascript:void(0);" class="<%=classStr%> <%=gnJiwonCalCnt != 0 ? "cal_dot" : ""%> date" id="<%=calDayStr%>a">
                                        ...
                                        <a href="javascript:void(0);" class="<%=classStr%> <%=listCount != 0 ? "cal_dot" : ""%> date" id="<%=calDayStr%>a"></a> --%>
                                            ...
                                            <ul class="">
                                                ...
                                                <%
                                                    if(gnJiwonCalList!=null&&gnJiwonCalList.size() > 0){
                                                    	int gnJiwonCalCount = 1;
                                                    	for(Map<String, Object> ob : gnJiwonCalList){
                                                            String busiSupportCd = getString(ob,"BUSI_SUPPORT_CD");
                                                            String busiNm = getString(ob,"BUSI_SUPPORT_NM");
                                                            String regStartDt = getString(ob, "REG_START_DT");
                                                            String regEndDt = getString(ob, "REG_END_DT");
                                                            
                                                            try {
                                                                if(smdf.parse(calDayStr).compareTo(smdf2.parse(regStartDt)) >= 0 && smdf2.parse(regEndDt).compareTo(smdf.parse(calDayStr)) >= 0){
                                                                    if(gnJiwonCalCount <= 2){
                                                %>
						                                                <li>
						                                                    <a href="/index.gyeong?&menuCd=<%=viewMenuCd%>&busiSupportCd=<%=busiSupportCd%>" class="detail_<%=busiTypeColor(getString(ob, "BUSI_CATE"),"main")%>">
						                                                        <p><%=busiNm %></p>
						                                                    </a>
						                                                </li>
                                                <%					}
                        ...
                        <div class="calendar__color">
                            <ul>
                                <li class="cal_color1">기술</li>
                                <li class="cal_color2">금융</li>
                                <li class="cal_color3">수출</li>
                                <li class="cal_color4">인력</li>
                                <li class="cal_color5">창업</li>
                                <li class="cal_color6">경영</li>
                                <li class="cal_color7">내수</li>
                ...
                <div class="brd2-conts">
                    ...
                    <div class="brd2-con promote">
                        ...
                        <ul>
                            <li>
                                <a>
                                    <h4 class="tit">제27회 중소기업대상 수상기업</h4>
                                    <span class="detail">자세히 보기</span>
                                </a>
                            </li>
                            <li>
                                <a>
                                    <h4 class="tit">K-디자인 어워드 수상작</h4>
                                    <span class="detail">자세히 보기</span>
                                </a>
                            </li>
                        </ul>
                        <!-- <ul>
                            <%
                                if(giupList !=null && giupList.size()>0){
                                for(Map<String,Object> ob3 : giupList){
                                    String thumbnail = "/images/micro/forestrecreation/content/noimage.jpg";
                                    String youtubeLink = getString(ob3, "TMP_FIELD10");
                                    String fileMask = getString(ob3, "FILE_MASK");
                                    if(!"".equals(fileMask)){
                                        thumbnail = "/upload_data/board_data/" + getString(ob3, "BOARD_ID") + "/" + fileMask;
                                    }else{
                                        if(!"".equals(youtubeLink)) {
                                            try {
                                                thumbnail = "https://i.ytimg.com/vi/" + youtubeLink.split("be/")[1] + "/0.jpg";
                            ...
                            %>
                            <li>
                                <a href="<%=giupViewPage%>&dataSid=<%=getString(ob3,"DATA_SID")%>&startPage=1" class="clearfix">
                                    <div class="img_box">
                                        <img src="<%=thumbnail%>" alt="" />
                                    </div>
                                    <div class="txt_box">
                                        <h4 class="ellipsis">
                                            <%=getString(ob3,"DATA_TITLE") %>
                                        </h4>
                                        <i><img src="/gnCorp119/images/common/board_ico_new.png" alt="새글"></i>
                                    </div>
                                </a>
